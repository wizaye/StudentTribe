  ...
  const savePin=(id)=>{
    if (!alreadySaved){
      ...
      client.patch(id).setIfMissing({save:[]}).insert('after','save[-1]',[{
        ...
        postedBy:{
          _type:'postedBy',_ref:user?.googleId
        ...
        {postHovered && (
          ...
          >
            <div className='flex items-center justify-between'>
              ...
              {alreadySaved? (
                <button type='button' className='bg-gray-500 opacity-70 hover:100 text-white font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none'>
                  {save?.length}Saved
                </button>
              ) : (
                <button
            ...
            <div className='flex justify-between items-center gap-2 w-full'>
              ...
              {postedBy?._id===user?.googleId&&(
                <button
                  ...
                  className='bg-white-500 opacity-70 hover:opacity-100 text-dark font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none'
                >
                  <AiTwotoneDelete/>
